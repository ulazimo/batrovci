...
- **And keep them out of the bottom ~22%, where the home UI is.** `#room-bottom`
  (Play button + streak meter + hall dots) spans **78.1% → 100%** of the scene on
  the phone frame; the Play button itself is **79.8%–87.6%** vertically and
  **x 32.3%–67.7%**, the streak bar **89%–95.3%** and **x 22.6%–77.4%**. So the
  usable band for art is roughly **top 22% (HUD/fabs) … 78%**. `#room-bottom` is
  a fixed **181px**, so it is a *smaller* fraction of a taller scene (80.0% on
  the iPad frame) — the phone's 78.1% is the stricter line and the one to design
  against.
- **`#room-bottom` is a scrim, not an opaque bar** — the single easiest thing to
  get wrong here. Its background is
  `linear-gradient(0deg, rgba(10,6,24,.9), rgba(10,6,24,0))`: fully
  **transparent at its top edge**, only 90% dark at the very bottom. Only the
  Play button and streak bar are solid, and each spans part of the width. So an
  item under the button is genuinely hidden, but art elsewhere down there is
  merely dimmed and is plainly visible in the gutters either side. Never assume
  "below 78% ⇒ invisible" and skimp on art there; it shows.
- **A layer hall's items were authored before that line existed**, and every one
  of them has pieces below it (attic's rocking horse at 99.7% of the picture, the
  study's magnifying glass entirely inside the band). The fix is not to move
  them — a `kind: 'layer'` canvas is baked — but to make the *picture taller at
  the bottom*, which pushes everything already in frame up the screen. Padding
  each `<hall>/scene/*.png` with the same rows keeps every item glued to its
  furniture, and **`collections.json` needs no edit**, since a layer slot ignores
  `left`/`bottom`/`h`. `art/tools/extend_backdrops.py` does all of this; read
  `art/tools/RUNLOG-extend-backdrops.md` before touching backdrop dimensions.
  Consequence: **a backdrop is no longer guaranteed to be 576×1024.** Extended
  halls are 576×1084 … 576×1358, each hall its own aspect — fine, because
  `syncBackdropBox()` reads `naturalWidth`/`naturalHeight` per image, but any new
  script that hardcodes the old size (as `wire_halls.py` still does) is wrong.
  **A newly authored hall arrives un-extended** and so lands squarely behind the
  Play button — Wedding Chapel did, with `champagneflutes` at 91.8%. Run
  `extend_backdrops.py prep --halls <new-hall>` as the last step of adding one;
  `prep` merges into the existing manifest and reports only halls not yet applied.
