...
| `levels/levels_default.js` / `_short.js` / `_cleaning.js` / `_cleaningxl.js` / `_long.js` | — | Level definitions per **journey** (16 / 40 / 40 / 40 / 253 levels). `.json` twins exist for the editor. **`cleaningxl` is the live journey** — see §10. |
...
| `art/reef/`, `art/shore/`, `art/snow/`, `art/childhood/`, `art/attic/`, `art/kitchen/`, `art/greenhouse/`, `art/musicroom/`, `art/workshop/` | — | Coral Reef / Sunny Shore / Snow Day / Childhood + the five interior-room halls — **transparent PNGs** (Layer AI generated, keyed off white locally). Same dual use as the SVGs; `view` is the pixel size. |
| `art/tools/` | — | The art pipeline. `key_white.py` keys Layer's white-background renders to tight transparent PNGs (Layer's own `remove_background` is silently ignored in this workspace); `wire_halls.py` registers the five interior-room halls into both `collections` twins. Both document *why* each step exists — read them before generating a new hall. |
...
- Today: **12 halls** — Childhood (image backdrop + tableau layers, levels 1–4),
  Music Hall (CSS theme, 5–9), Green Pasture (CSS theme, 10–14), Coral Reef (image
  backdrop + placed items, 15–19), Sunny Shore (20–24), Snow Day (25–29), Bedroom
  (30–33), Cozy Attic (34–38), Grandma's Kitchen (39–43), Victorian Greenhouse
  (44–48), Music Room (49–53), Toy Workshop (54–58) — and `boardArt` for
  `cleaningxl` **1–58**.
  The five interior-room halls (Attic → Workshop) are image backdrops with placed
  items and are the only ones carrying **`floatPx: 0`** — their items are pinned to
  painted surfaces (a crate top, a counter, a shelf), where a bob visibly lifts
  them off the plane.
- **A `levelId` may appear in only one hall.** `slotLevelIndex` maps id → index and
  ...
  authored for 30–54 and shifted to **34–58** when Bedroom took 30–33; the `first`
  level per hall lives in `art/tools/wire_halls.py`.
...
- The journey is 40 levels, so **levels 41–58 are unreachable** until it is
  extended: Greenhouse, Music Room and Toy Workshop are art and data waiting for
  levels, and Grandma's Kitchen is only reachable as far as level 40.
...
`COLLECTIONS.halls` (§3) — twelve of them, 4–5 slots each, covering levels 1–58 (the
...
Note: halls cover levels 1–58 but the live journey only has 40, so the last halls are
authored ahead of the levels that unlock them; `slotLevelIndex` returns −1 for an id
