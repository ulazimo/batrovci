...
COLLECTIONS = {
  ...
  items:    { guitar: { name, file, view: {w,h} }, … },        // view = the SVG's viewBox
  halls:    [ { id, name, theme, backdrop?, slots: [ { item, levelId, left, bottom, h, pw? } ] } ],
...
- Adding a hall that reuses an existing `theme` is pure data. A genuinely new look
  needs a new `.theme-<id>` CSS preset **and** a `COLLECTIONS.themes` entry;
  `pedestals` picks `.spot-pedestal` vs `.spot-shadow`. A hall with `backdrop` set
  shows `#room-backdrop` and hides the procedural `#room-bg` scenery entirely.
- Today: 2 halls (Music Hall, Green Pasture) covering levels 1–10, and `boardArt`
  for `cleaningxl` levels 1–10 + 38–40.
