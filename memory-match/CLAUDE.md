...
| `art/reef/`, `art/shore/`, `art/snow/`, `art/childhood/` | — | Coral Reef / Sunny Shore / Snow Day / Childhood collectibles — **transparent PNGs** (Layer AI generated, keyed off white locally). Same dual use as the SVGs; `view` is the pixel size. |
...
- Today: **6 halls** — Childhood (image backdrop + tableau layers, levels 1–4),
...
`COLLECTIONS.halls` (§3) — six of them, 4–5 slots each, covering levels 1–29 (the
...
Note: halls only cover levels 1–29, so for level 30+ `currentHallIndex()` falls back
