...
- Today: **11 halls** — Sunny Shore (image backdrop + placed items, levels 1–5),
  Snow Day (6–10), Bedroom (tableau layers, 11–14), Cozy Attic (**tableau layers**,
  15–19), Grandma's Kitchen (20–24), Victorian Greenhouse (25–29), Music Room
  (30–34), Toy Workshop (35–39), Cosy Library (40–44), Artist's Studio (45–49),
  Sewing Room (50–54) — and `boardArt` authored for `cleaningxl` **1–41**, of which
  **1–35 are live** (the journey is mid-rebuild; see below).
  Commit `bc2a31d` removed Childhood, Music Hall, Green Pasture and Coral Reef and
  renumbered everything down; anything citing levels 34–58 or 12 halls is stale.
  The eight interior-room halls (Attic → Sewing Room) are **`kind: 'layer'` halls**
  as of 2026-07-29 — their items are painted into the room by a Layer image-edit and
  detached by pixel-diff, so each carries a real contact shadow and correct light.
  They therefore have `shadow: false`, no `floatPx`, and no pedestals: `shadow`,
  `glow` and `floatPx` are all inert for a layer slot. See
  `art/SCENE-LAYERS-PLAN.md`, `art/tools/RUNLOG.md` (halls 4–8) and
...
- **The halls run ahead of the levels, and by how much moves.** Halls cover levels
  **1–54**; `cleaningxl` is mid-rebuild and currently ends at **35**, so halls 1–7
  are fully reachable, Toy Workshop reveals only its first slot (35), and Cosy
  Library / Artist's Studio / Sewing Room reveal nothing yet. That degrades
  gracefully by design — `slotLevelIndex` returns −1 and the slot is simply absent,
  no throw — so authoring hall art early is safe. Two consequences worth knowing:
  `add_halls_9_11.py` writes `boardArt` **only for levels that exist when it runs**,
  because fabricating entries for 13 unauthored levels would be dead data that looks
  live; but entries **orphaned later** by a level deletion are left in place (36–41
  are orphaned right now), since they are inert and correctly pre-wire those levels
  if they come back. Neither case is a bug — just re-run the script after authoring
  levels 42+ to fill the gap.
...
- **cleaningxl** — 35 levels (and still being re-authored). **This is the live journey** (`MAIN_JOURNEY`,
...
Note: halls cover levels 1–54 but the live journey currently has 35, so the last halls
are authored ahead of the levels that unlock them; `slotLevelIndex` returns −1 for an id
